...
MqttKit::MqttKit(QObject *parent) : QObject(parent)
{
...
//				2.可使用局部或全局数组来指定内存
//==========================================================
void MqttKit::MQTT_NewBuffer(MQTT_PACKET_STRUCTURE *mqttPacket, uint32 size)
{
...
//	说明：
//==========================================================
void MqttKit::MQTT_DeleteBuffer(MQTT_PACKET_STRUCTURE *mqttPacket)
{
...
int32 MqttKit::MQTT_DumpLength(size_t len, uint8 *buf)
{
...
int32 MqttKit::MQTT_ReadLength(const uint8 *stream, int32 size, uint32 *len)
{
...
//	说明：
//==========================================================
uint8 MqttKit::MQTT_UnPacketRecv(uint8 *dataPtr)
{
...
//	说明：
//==========================================================
uint8 MqttKit::MQTT_PacketConnect(const int8 *user, const int8 *password, const int8 *devid,
...
//	说明：
//==========================================================
uint1 MqttKit::MQTT_PacketDisConnect(MQTT_PACKET_STRUCTURE *mqttPacket)
{
...
//	说明：
//==========================================================
uint8 MqttKit::MQTT_UnPacketConnectAck(uint8 *rev_data)
{
...
//	说明：
//==========================================================
uint1 MqttKit::MQTT_PacketSaveData(const int8 *pro_id, const char *dev_name,
...
//	说明：
//==========================================================
uint1 MqttKit::MQTT_PacketSaveBinData(const int8 *name, int16 file_len, MQTT_PACKET_STRUCTURE *mqttPacket)
{
...
//	说明：
//==========================================================
uint8 MqttKit::MQTT_UnPacketCmd(uint8 *rev_data, int8 **cmdid, int8 **req, uint16 *req_len)
{
...
//	说明：
//==========================================================
uint1 MqttKit::MQTT_PacketCmdResp(const int8 *cmdid, const int8 *req, MQTT_PACKET_STRUCTURE *mqttPacket)
{
    ...
    int8 *payload = reinterpret_cast<int8*>(MQTT_MallocBuffer(cmdid_len + 7));
    if(payload == NULL)
        return 1;
...
//	说明：
//==========================================================
uint8 MqttKit::MQTT_PacketSubscribe(uint16 pkt_id, enum MqttQosLevel qos, const int8 *topics[], uint8 topics_cnt, MQTT_PACKET_STRUCTURE *mqttPacket)
{
...
//	说明：
//==========================================================
uint8 MqttKit::MQTT_UnPacketSubscribe(uint8 *rev_data)
{
...
//	说明：
//==========================================================
uint8 MqttKit::MQTT_PacketUnSubscribe(uint16 pkt_id, const int8 *topics[], uint8 topics_cnt, MQTT_PACKET_STRUCTURE *mqttPacket)
{
...
//	说明：
//==========================================================
uint1 MqttKit::MQTT_UnPacketUnSubscribe(uint8 *rev_data)
{
...
//	说明：
//==========================================================
uint8 MqttKit::MQTT_PacketPublish(uint16 pkt_id, const int8 *topic,
...
//	说明：
//==========================================================
uint8 MqttKit::MQTT_UnPacketPublish(uint8 *rev_data, int8 **topic, uint16 *topic_len, int8 **payload, uint16 *payload_len, uint8 *qos, uint16 *pkt_id)
{
    ...
    *topic_len = (uint16)msgPtr[0] << 8 | msgPtr[1];
    ...
    switch(*qos)
    {
        case MQTT_QOS_LEVEL0:									// qos0 have no packet identifier
            ...
            *topic = reinterpret_cast<int8*>(MQTT_MallocBuffer(*topic_len + 1));			//为topic分配内存
            if(*topic == NULL)
                return 255;
            ...
            *payload = reinterpret_cast<int8*>(MQTT_MallocBuffer(*payload_len + 1));
        ...
        case MQTT_QOS_LEVEL2:
            ...
            *payload = reinterpret_cast<int8*>(MQTT_MallocBuffer(*payload_len + 1));		//为payload分配内存
            if(*payload == NULL)								//如果失败
            {
                MQTT_FreeBuffer(*topic);						//则需要把topic的内存释放掉
                return 255;
...
//==========================================================
uint1 MqttKit::MQTT_PacketPublishAck(uint16 pkt_id, MQTT_PACKET_STRUCTURE *mqttPacket)
{
...
//	说明：
//==========================================================
uint1 MqttKit::MQTT_UnPacketPublishAck(uint8 *rev_data)
{
...
//==========================================================
uint1 MqttKit::MQTT_PacketPublishRec(uint16 pkt_id, MQTT_PACKET_STRUCTURE *mqttPacket)
{
...
//	说明：
//==========================================================
uint1 MqttKit::MQTT_UnPacketPublishRec(uint8 *rev_data)
{
...
//==========================================================
uint1 MqttKit::MQTT_PacketPublishRel(uint16 pkt_id, MQTT_PACKET_STRUCTURE *mqttPacket)
{
...
//	说明：
//==========================================================
uint1 MqttKit::MQTT_UnPacketPublishRel(uint8 *rev_data, uint16 pkt_id)
{
...
//	说明：		当收到的Publish消息的QoS等级为2时，先收到rec，再回复rel
//==========================================================
uint1 MqttKit::MQTT_PacketPublishComp(uint16 pkt_id, MQTT_PACKET_STRUCTURE *mqttPacket)
{
...
//	说明：
//==========================================================
uint1 MqttKit::MQTT_UnPacketPublishComp(uint8 *rev_data)
{
...
//	说明：
//==========================================================
uint1 MqttKit::MQTT_PacketPing(MQTT_PACKET_STRUCTURE *mqttPacket)
{
